...
.db-input-short  { width: 160px; }
.db-input-medium { width: 280px; }
.db-input-long   { width: 400px; }
...
.db-table thead tr { background: linear-gradient(90deg, #273449, #5b8dee); }
.db-table thead tr th { color: #ffffff; font-weight: 600; letter-spacing: 0.3px; }
...
.db-print-header h2 {
  font-family: 'Poppins', sans-serif;
  font-size: 1.1rem; font-weight: 700; color: #273449; margin: 0 0 4px 0;
}
.db-print-header p {
  font-family: 'Poppins', sans-serif;
...
const USER_OPTIONS = [
  { value: "",    label: "--Select--" },
  { value: "U01", label: "User 01" },
  { value: "U02", label: "User 02" },
  { value: "U03", label: "User 03" },
];

function AuditTrialReport() {
  const [form, setForm] = useState({
    reportType:     "SpecificUser",  // "SpecificUser" | "AllUser"
    userCode:       "",
    userName:       "",
    fromDate:       "26/07/2025",
    toDate:         "26/07/2025",
  ...
  const handleUserSelect = (e) => {
    ...
    const opt = USER_OPTIONS.find(u => u.value === val);
    setForm(prev => ({ ...prev, userCode: val, userName: opt ? opt.label : "" }));
    setFetched(false);
  };
  ...
  const validate = () => {
    if (!form.fromDate.trim() || !parseDate(form.fromDate)) return "From Date must be DD/MM/YYYY.";
    if (!form.toDate.trim()   || !parseDate(form.toDate))   return "To Date must be DD/MM/YYYY.";
    if (form.reportType === "SpecificUser" && !form.userCode) return "Please select a user.";
    return null;
  };

  const buildQuery = () => new URLSearchParams({
    reportType:     form.reportType,
    userCode:       form.reportType === "AllUser" ? "All" : form.userCode,
  ...
  const fetchData = async (mode) => {
    ...
    try {
      const res  = await fetch(`${API_BASE_URL}/api/audit-trial-report?${buildQuery()}`);
      if (!res.ok) {
  ...
  const handleDownload = async () => {
    ...
    a.download = `AuditTrialReport_${form.fromDate.replace(/\//g,"-")}_${form.toDate.replace(/\//g,"-")}.csv`;
  ...
  const handleExit = () => {
    setForm({ reportType:"SpecificUser", userCode:"", userName:"", fromDate:"26/07/2025", toDate:"26/07/2025", textReportName:"" });
    ...
    <>
      ...
      <div className="db-wrapper">
        <div className="db-card no-print">
          ...
          {/* HEADER */}
          <div className="db-header">Audit Trial Report</div>

          {/* BODY */}
          <div className="db-body">

            {/* REPORT TYPE — vertical radios */}
            <div className="db-row db-row-top">
              <label className="db-label">Report Type</label>
              <div className="db-radio-col">
                <label className="db-radio-label">
                  <input type="radio" name="reportType" value="SpecificUser"
                    checked={form.reportType === "SpecificUser"} onChange={handleChange} />
                  Specific User
                </label>
                <label className="db-radio-label">
                  <input type="radio" name="reportType" value="AllUser"
                    checked={form.reportType === "AllUser"} onChange={handleChange} />
                  All User
                </label>
              </div>
            </div>
            ...
            <div className="db-row">
              <label className="db-label">User List</label>
              <select
                className="db-select"
                name="userCode"
                value={form.userCode}
                onChange={handleUserSelect}
                disabled={form.reportType === "AllUser"}
              >
                {USER_OPTIONS.map(o => (
              ...
              <input
                ...
                type="text"
                name="userName"
                value={form.reportType === "AllUser" ? "" : form.userName}
                readOnly
                placeholder=""
              />
            </div>
            ...
            <div className="db-row">
              ...
              <input className="db-input db-input-medium" type="text"
                name="fromDate" value={form.fromDate}
                onChange={handleChange} placeholder="DD/MM/YYYY" />
              <label className="db-label" style={{ minWidth: 80 }}>To Date</label>
              <input className="db-input db-input-medium" type="text"
            ...
            {/* TEXT REPORT NAME */}
            <div className="db-row">
              <label className="db-label">Enter Text Report Name</label>
              <input className="db-input db-input-long" type="text"
          ...
          <div className="db-footer">
            <button className="db-btn db-btn-blue" onClick={handleReport} disabled={loading}>
              {loading && printMode === "report" ? "Loading..." : "Report"}
            </button>
            <button className="db-btn db-btn-exit" onClick={handleExit}>Exit</button>
            <button className="db-btn db-btn-blue" onClick={handleView} disabled={loading}>
            ...
            <button className="db-btn db-btn-blue" onClick={handlePrint} disabled={loading}>
        ...
        {fetched && reportData.length > 0 && (
          <div className="db-table-wrapper">
            <div className="print-only db-print-header">
              ...
              <p>From: {form.fromDate} &nbsp;|&nbsp; To: {form.toDate} &nbsp;|&nbsp; Printed: {new Date().toLocaleDateString()}</p>
            </div>
